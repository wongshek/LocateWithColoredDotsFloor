...
void ImgPreProcess(Mat &img, Mat &cannyImg)
{
	...
	pyrDown(img, img, Size(img.cols / 2, img.rows / 2));
	imwrite("./data/resizeImg.png", img);

	//预处理
	GaussianBlur(img, img, Size(3, 3), 0); //高斯模糊
	imwrite("./data/gaussblurImg.png", img);
	...
	imwrite("./data/cannyImg.png", cannyImg);

	//膨胀
	Mat element = getStructuringElement(MORPH_RECT, Size(3, 3));
	dilate(cannyImg, cannyImg, element);
	imwrite("./data/dilateImg.png", cannyImg);
	...
	imwrite("./data/eraseImg.png", cannyImg);
...
void GetLines(Mat img, Mat cannyImg, vector<Vec4i> &extendOptLines)
{
	...
	imwrite("./data/houghImg.png", drawImg);
	...
	imwrite("./data/extendLinesImg.png", drawImg);
	...
	imwrite("./data/optLinePts.png", linePts);
	...
	imwrite("./data/optLinesImg.png", drawImg);
	...
	imwrite("./data/extendOptLinesImg.png", drawImg);
}
...
void GetCornerCodingPts(Mat img, vector<Vec4i> extendOptLines, vector<Point2i> &cornerPoints, vector<Vec4i> &codingPoints, vector<Vec3i> &ccFilterPts)
{
	...
	imwrite("./data/cornerPtsImg.png", drawImg);

	//转换颜色空间至HSV
	Mat hsvImg;
	cvtColor(img, hsvImg, COLOR_BGR2HSV);
	imwrite("./data/hsvImg.png", hsvImg);
	...
	imwrite("./data/firstCodingPtsImg.png", drawImg);
	...
	imwrite("./data/CodingPtsImg.png", drawImg);
...
{
	/**********读取配置文件**********/
	FileStorage config("./data/config.yaml", FileStorage::READ);
